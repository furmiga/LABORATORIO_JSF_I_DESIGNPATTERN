...
public class CandleFactoryTest {
	...
	public void sequenciaDeNegociacoesSimples() {
		...
		Candle candle = fabrica.gerarCandleParaData(negociacoes, hoje);
	...
	@Test
	public void geraCandlstickComApenaUmaNegociacao() {
		LocalDateTime hoje = LocalDateTime.now();
		Negociacao negociacao1 = new Negociacao(40.0, 100, hoje);
	...
	@Test
	public void negociacaoDeTresDiasDiferentesTresGeraCandlesDiferentes(){
		LocalDateTime hoje = LocalDateTime.now();
		...
		Negociacao negociacao7 = new Negociacao(10.0, 20, depois);
		...
		List<Candle> candlesSticks = fabrica.constroiCandles(negociacoes);
		
		Assert.assertEquals(3 ,candlesSticks.size());
		
		Assert.assertTrue(negociacoes.get(0).isMesmoDia(candlesSticks.get(0).getData()));
		Assert.assertTrue(negociacoes.get(3).isMesmoDia(candlesSticks.get(1).getData()));
		Assert.assertTrue(negociacoes.get(5).isMesmoDia(candlesSticks.get(2).getData()));
		
		Assert.assertEquals(6000.0,candlesSticks.get(0).getVolume(), 0.0000001);
		Assert.assertEquals(50.0,candlesSticks.get(0).getMinimo(), 0.0000001);
		Assert.assertEquals(150.0,candlesSticks.get(0).getMaximo(), 0.0000001);
		Assert.assertEquals(50.0,candlesSticks.get(0).getAbertura(), 0.0000001);
		Assert.assertEquals(150.0,candlesSticks.get(0).getFechamento(), 0.0000001);
